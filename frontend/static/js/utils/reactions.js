				...
				.then(async (response) => {
					...
					if (response.ok) {
						...
						const reactionData = await fetch(
							`https://localhost:8080/reaction?rowID=${parseInt(
								ReactionID
							)}&reactionParentClass=${Type}`,
							{
						...
						if (reactionData.ok) {
							...
							const otherButton = postElement.querySelector(
								`.reaction-button[reaction-action="${otherButtonAction}"]`
							)
							if (Action == "like") {
							...
							console.log(
								"Error fetching reaction data:",
								reactionData.statusText
							)
						}
					} else {
